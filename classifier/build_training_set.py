...
def performance_till_date(team, seasons_to_consider, season, match_day, match_history, standings_history):
    ...
    # first the stats from previous seasons
        ...
        cnt += 1
        tot_games += apply_weight(MDS_PER_SEASON,season_index - (i-1))
    ...
    if(match_day > 1):
       ...
       tot_games += apply_weight(match_day - 1,0)
...
def get_standing(team, season, match_day, match_history, standings_history):
...
def matchups_till_date(team1, team2, seasons_to_consider, season, match_day, match_history, standings_history):
    ...
    def compute_parameters(matches, team1, team2, season):
        ...
        num_matches = apply_weight(len(common), season_index - season_id)
    ...
    for season_id in range(season_index - seasons_to_consider, season_index + 1):
        ...
        if season_id == season_index:
            ...
            common, num_matches = compute_parameters(matches, team1, team2, season_index - season_id)
            common_matches_data = pd.concat([common_matches_data, common])
        else:
            common, num_matches = compute_parameters(matches, team1, team2, season_index - season_id)
            common_matches_data = pd.concat([common_matches_data, common])
    ...
    # Write result
    results = common_matches_data.sum()/tot_matches
    ...
    for season_id in range(season_index - seasons_to_consider, season_index + 1):
        ...
        if season_id == season_index:
            if match_day > 1:
                ...
                total_matches += apply_weight(match_day - 1, season_index - season_id)
        else:
            # Get last match day standings
            standings = standings.loc[standings['match_day'] == MDS_PER_SEASON]
            total_matches += apply_weight(MDS_PER_SEASON, season_index - season_id)
...
def build_vector_for_match(team1, team2, seasons_to_consider, season, match_day, match_history, standings_history):
    ...
    standing = pd.Series({'t1_standing' : get_standing(team1, season, match_day, match_history, standings_history),
                          't2_standing': get_standing(team2, season, match_day, match_history, standings_history)})
    perf1 = performance_till_date(team1, seasons_to_consider, season, match_day, match_history, standings_history)
    ...
    perf2 = performance_till_date(team2, seasons_to_consider, season, match_day, match_history, standings_history)
    ...
    ha_perf1 = home_away_performance_till_date(team1, seasons_to_consider, season, match_day, match_history, standings_history)
    ha_perf1.rename(index={'avg_home_points' : 't1_avg_home_pts_tot',
                           'avg_away_points' : 't1_avg_away_pts_tot'}, inplace=True)
    ha_perf2 = home_away_performance_till_date(team2, seasons_to_consider, season, match_day, match_history, standings_history)
...
def build_training_set(match_history, standings_history):
    ...
    for season in seasons[SEASONS_TO_CONSIDER : NUM_SEASONS]:
        ...
        for match_day in range(1, MDS_PER_SEASON + 1):
            ...
            for match in md.iterrows():
                ...
                this_game = build_vector_for_match(team1, team2, SEASONS_TO_CONSIDER, season, match_day, match_history, standings_history)
...
def main():
    ...
    match_history, standings_history = read_stats()
 
    # home_away_performance_till_date(9, 2, "2012_2013", 38, match_history, standings_history)
    build_training_set(match_history, standings_history)
