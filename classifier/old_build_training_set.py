...
def build_team_stats(team1, team2, season, match_day, seasons_to_consider=2):
...
# def build_training_set(num_seasons, seasons_to_consider, match_history):
# 	"""Creates a training example for each of the matches in the match history"""

# 	NUM_PARAMS = 7
# 	training_set = np.zeros(MATCHES_PER_SEASON * (NUM_SEASONS - seasons_to_consider), 2 * NUM_PARAMS + 1)

# 	for season in range(seasons_to_consider, num_seasons):
# 		for match_day in range(MDS_PER_SEASON):
# 			for match in range(MATCHES_PER_MD):
# 				example = np.array(1, 2 * NUM_PARAMS + 1)
# 				team1 = match_history[season, match_day * MATCHES_PER_MD + match, 1]
# 				team2 = match_history[season, match_day * MATCHES_PER_MD + match, 2]
# 				np.insert(example, 0, build_team_stats(team1, team2, season, match_day, seasons_to_consider))
# 				np.insert(example, NUM_PARAMS, build_team_stats(team2, team1, season, match_day, seasons_to_consider))

# 				if(match_history[season, match_day * MATCHES_PER_MD + match, 3] > match_history[season, match_day * MATCHES_PER_MD + match, 4]):
# 					outcome = 1
# 				else if(match_history[season, match_day * MATCHES_PER_MD + match, 3] < match_history[season, match_day * MATCHES_PER_MD + match, 4]):
# 					outcome = -1
# 				else:
# 					outcome = 0
...
if(match_history[season, match_day * MATCHES_PER_MD + match, 3] > match_history[season, match_day * MATCHES_PER_MD + match, 4]):
	outcome = 1
elif(match_history[season, match_day * MATCHES_PER_MD + match, 3] < match_history[season, match_day * MATCHES_PER_MD + match, 4]):
	outcome = -1
else:
	outcome = 0

example = np.concatenate((build_team_stats(team1, team2, season, match_day, seasons_to_consider), \
					build_team_stats(team2, team1, season, match_day, seasons_to_consider), \
					np.arraoutcome))
